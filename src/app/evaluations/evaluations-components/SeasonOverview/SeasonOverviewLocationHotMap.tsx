...
const SeasonOverviewLocationHotMap: React.FC = () => {
  ...
  const modelPerformanceData = useMemo(() => {
    if (seasonOverviewData) {
      ...
      const modelData = locationMapData[mapSelectedModel] || {};

      // Calculate averages across selected horizons for each location
      Object.entries(modelData).forEach(([locationCode, horizonData]) => {
        let totalScore = 0;
        let totalCount = 0;
        ...
        if (totalCount > 0) {
          const locId = locationCode.toString().padStart(2, "0");
          locationPerformanceMap.set(locId, totalScore / totalCount);
        }
      });
...
export default SeasonOverviewLocationHotMap;
