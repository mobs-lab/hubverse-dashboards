...
interface IQRData {
  model: string;
  // Dynamic intervals mapped to visual elements
  lowerWhisker: number; // e.g. q05
  lowerBox: number; // e.g. q25
  median: number;
  upperBox: number; // e.g. q75
  upperWhisker: number; // e.g. q95
  count: number;
  // Metadata about which levels were used
  boxLevel: string;
  whiskerLevel: string;
}

const SeasonOverviewAllLocationBoxPlot: React.FC<
...
> = ({ type }) => {
  ...
  const updateTooltip = useCallback(
    (
      ...
      tooltip.selectAll('*').remove();
        ...
        // Dynamically label the percentiles based on which intervals were used
        // Box = 50% interval = 25th - 75th percentiles
        // Whiskers = 95% interval = 2.5th - 97.5th percentiles (approx)

        const boxAlpha = 1 - parseInt(data.boxLevel) / 100;
        const whiskerAlpha = 1 - parseInt(data.whiskerLevel) / 100;

        // Calculate approx percentile labels
        const pLowerWhisker = `${((whiskerAlpha / 2) * 100).toFixed(1)}%`;
        const pLowerBox = `${((boxAlpha / 2) * 100).toFixed(1)}%`;
        const pUpperBox = `${((1 - boxAlpha / 2) * 100).toFixed(1)}%`;
        const pUpperWhisker = `${((1 - whiskerAlpha / 2) * 100).toFixed(1)}%`;

        const metrics = [pLowerWhisker, pLowerBox, 'Median', pUpperBox, pUpperWhisker];
        const values = [
          data.lowerWhisker,
          data.lowerBox,
          data.median,
      ...
      tooltip
        .attr('transform', `translate(${position[0] + xOffset},${position[1] + yOffset})`)
        .style('opacity', 1);
    },
    [type, dimensions]
  );
  ...
  const renderChart = useCallback(() => {
    ...
    const maxValue = Math.max(...data.map((d) => d.upperWhisker)) * 1.1; // Add 10% padding
    // const xScale = d3.scaleLinear().domain([0, maxValue]).range([0, innerWidth]);

    const dataRange = {
      min: Math.min(...data.map((d) => d.lowerWhisker)),
      max: Math.max(...data.map((d) => d.upperWhisker)),
    ...
    data.forEach((d, i) => {
      ...
      boxGroup
        .append('line')
        .attr('x1', xScale(d.lowerWhisker))
        .attr('x2', xScale(d.upperWhisker))
      ...
      boxGroup
        .append('rect')
        .attr('x', xScale(d.lowerBox))
        .attr('width', Math.max(0, xScale(d.upperBox) - xScale(d.lowerBox)))
        .attr('y', y)
        ...
        .on('mouseover', function (event) {
          ...
          let tooltip:
            ...
            | d3.Selection<d3.BaseType, unknown, null, undefined> = svg.select('.iqr-tooltip');
          if (tooltip.empty()) {
        ...
        .on('mousemove', function (event) {
          ...
          updateTooltip(
            tooltip as unknown as d3.Selection<SVGGElement, unknown, null, undefined>,
            d,
            [mouseX, mouseY],
            direction
          );
...
export default SeasonOverviewAllLocationBoxPlot;
