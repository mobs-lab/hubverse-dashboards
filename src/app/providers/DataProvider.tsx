    ...
    async (currentSeasonId: string, metadata: any) => {
      ...
      try {
        if (metadata?.fullRangeSeasons) {
          const previousSeasons = metadata.fullRangeSeasons.filter(
            (s: any) => s.seasonId !== currentSeasonId
          );

          for (const season of previousSeasons) {
            try {
              const seasonData = await fetchSeasonData(season.seasonId, false, [
                'groundTruthData',
                'predictionsData',
                'nowcastTrendsData',
              ]);
      ...
      if (auxiliaryData.metadata) {
        ...
        if (metadata.fullRangeSeasons) {
          ...
          const evalSOTimeRangeOptions: EvaluationSeasonOverviewTimeRangeOption[] = [
            ...seasonOptions.map((season: ForecastPeriodOption) => ({
              name: season.forecastPeriodID,
        ...
        if (metadata.defaultSeasonTimeValue && metadata.fullRangeSeasons) {
          const defaultOption = metadata.fullRangeSeasons.find(
            (s: any) => s.timeValue === metadata.defaultSeasonTimeValue
          );
